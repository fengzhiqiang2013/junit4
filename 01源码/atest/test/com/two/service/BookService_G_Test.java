...
@PrepareForTest({OrganizationUtil.class, FilesUtil.class, BookServiceImpl.class, AuthorServiceImpl.class})
public class BookService_G_Test {
	@Test
	public void testSaveBook() throws Exception{
		...
		//多级模拟
		AuthorServiceImpl authorService = Mockito.mock(AuthorServiceImpl.class);
		IAuthorDao authorDaoTwo =  Mockito.mock(AuthorDaoImpl.class);
		
		//模拟内部的saveBook(book)方法，queryAuthorByIdAndDate(id, date)方法
		//静态OrganizationUtil.getCurrUserId()方法,静态FilesUtil.saveFiles(files)方法
		PowerMockito.whenNew(Date.class).withNoArguments().thenReturn(date);
		Mockito.doReturn(author).when(authorDao).queryAuthorByIdAndDate(Mockito.anyString(), (Date)Mockito.anyObject());
		...
		//多级模拟
		PowerMockito.doCallRealMethod().when(authorService).saveAuthor(author);
		Mockito.doNothing().when(authorDaoTwo).saveAuthor((TAuthor)Mockito.anyObject());
		...
		ReflectionTestUtils.setField(authorService, "authorDao", authorDaoTwo);
		
		//执行
		String id = bookService.saveBook_G(book, authorId, files);
